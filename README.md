...
* `jsonWebTokens` - object containing information on how the jwt's should be constructed
	...
	* `includeUserInJwtResponse` - when JWT is the default response for succesfull log-in you can return the user along with the token by setting this to true - cuts down round trips
