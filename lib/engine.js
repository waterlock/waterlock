...
module.exports = function(){
    ...
    findAuth: function(criteria, cb){
      var self = this;
      waterlock.Auth.find(criteria).limit(1).populate('user')
      .exec(function(err, auth){
        if (auth instanceof Array) auth = auth[0];
    ...
    _attachAuthToUser: function(auth, cb){
      ...
      // create the user
      if(!auth.user){
        waterlock.User.create({auth:auth.id}).meta({fetch: true}).exec(function(err, user){
          if(err){
            waterlock.logger.debug(err);
            return cb(err);
          }

          waterlock.Auth.update(auth.id, {user:user.id}).meta({fetch: true}).exec(function(err, auth){
            if(err){
              waterlock.logger.debug(err);
              return cb(err);
            }
    ...
    findOrCreateAuth: function(criteria, attributes, cb){
      ...
      .exec(function(err, newAuth){
        ...
        waterlock.Auth.find(newAuth.id).limit(1).populate('user')
        .exec(function(err, auth){
          if(err){
            waterlock.logger.debug(err);
            return cb(err);
    ...
    attachAuthToUser: function(attributes, user, cb){
      ...
      waterlock.User.find(user.id).limit(1).exec(function(err, user){
        if(err){
          waterlock.logger.debug(err);
          return cb(err);
        }
        ...
        if(user.auth){
          ...
          waterlock.Auth.find(user.auth).limit(1).exec(function(err, auth){
            if(err){
              waterlock.logger.debug(err);
              return cb(err);
            }
