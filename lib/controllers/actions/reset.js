...
function sendEmail(req, res, User, ResetToken, params){
  User.findOne({email: params.email}).exec(function(err, u){
    if(u){
      ResetToken.create({owner: u.id}).exec(function(err, t){
        if(err){
          console.log(err);
        }
        User.update({id: u.id}, {resetToken: t.id}).exec(function(err){
          if(err){
            console.log(err);
          }
          res.json(200);
...
function issuePasswordReset(req, res, User, ResetToken, params){
  User.findOne(req.session.resetToken.owner).populate('resetToken').exec(function(err, u){
    if(err){
      console.log(err);
    }
    ...
    User.update({id:u.id},{resetToken: null, password: params.password}).exec(function(err, u){
      if(err){
        console.log(err);
      }

      // destroy the token
      ResetToken.destroy(tokenId).exec(function(err){
        if(err){
          console.log(err);
        }
...
function validateToken(req, res, User, ResetToken, params){
  User.findOne(params.id).populate('resetToken').exec(function(err, u){
